...
public class DBHelper
    ...
    private static final String DATABASE_NAME = "TamilKannada04feb21.sqlite";
   ...
   /* public String getPraises(String id) {
    ...
    public String getKannadaPraises()
    {
        File localFile = ctx.getDatabasePath(DATABASE_NAME);
        try {
            if (!localFile.exists()) {
            ...
            System.out.println("Error in getKannadaPraises" + e);
        }
        StringBuffer sf = new StringBuffer();
        Cursor localCursor = getReadableDatabase().rawQuery("SELECT id,praises,ref FROM kannadaPraises where id between " + 1 + " and " + 1000, null);
        while (localCursor.moveToNext()) {
            sf.append("\n"+localCursor.getString(0) +".  " + localCursor.getString(1) + "  ( "+ localCursor.getString(2) +" )\n" ) ;
        }
        return sf.toString();
